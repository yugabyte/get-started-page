<template>
  <div>
    <h3 class="config-form-header">1. Generate config file</h3>
      ...
      <div class="form-control">
        <template v-if="this.code == 'azurerm'">
          <div class="row config-form-row">
            <q-input
              ...
              v-model="azureSubIdInput"
            />
            <q-input
              class="form-input"
              label="Azure Client ID"
              v-model="azureClientIdInput"
            />
          </div>
          <div class="row config-form-row">
            <q-input
        ...
        <template v-else-if="this.code == 'google'">
          <div>
            <q-input
              class="form-input"
              label="GCP Credential File"
              v-model="gcpCredentialFile"
            />
          </div>
          <div class="row config-form-row">
            <q-input
        ...
        <template v-else>
          ...
          <div class="row config-form-row">
            <q-input
              class="form-input"
              label="SSH Keypair"
              v-model="sshKeyPairInput"
            />
            <q-input
              class="form-input"
              label="SSH Key Path"
              v-model="sshKeyPathInput"
            />
          </div>
          <div class="row config-form-row">
            <q-input
    ...
    <h3 class="config-form-header">2. Create cluster</h3>
...
<script>
import terraformCode, {
  generateAwsConfig,
  generateAzureConfig,
  generateGCPConfig,
} from './snippets/terraformDeploy';
import CopyButton from './CopyButton.vue';

export default {
  name: 'TerraformForm.vue',
  data: function () {
    return {
      /* Input form bindings */
